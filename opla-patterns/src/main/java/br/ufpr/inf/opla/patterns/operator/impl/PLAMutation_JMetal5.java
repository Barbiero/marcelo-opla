...
/**
 * Implementaçao de {@link PLAMutation} para o jmetal5
 */
public class PLAMutation_JMetal5 implements MutationOperator<ArchitectureSolution> {

    private static final Logger LOGGER = LogManager.getLogger(PLAMutation_JMetal5.class);
    private final double probability;
    private JavaRandomGenerator rng = null;


    public PLAMutation_JMetal5(double probability) {
    ...
    private Class findOrCreateClassWithConcern(Package targetComp, Concern concern) {
        return targetComp.getAllClasses().stream()
                ...
                .findFirst()
                .orElseGet(() -> {
                    Class targetClass = targetComp.createClass("Class" + OPLAProblem.contClass, false);
                    OPLAProblem.contClass++;
     ...
     *
     * @see PLAMutation#isVarPointOfConcern
     * @see PLAMutation#isVariantOfConcern
     */
    ...
    private void moveOperationToComponent(Method method, Interface sourceInterface, Package targetComponent, Package comp, Architecture arch, Concern concern) {
        ...
        if (maybeTargetInterface.isPresent()) {
            ...
            targetInterface = targetComponent.createInterface("Interface" + OPLAProblem.contInterface);
            OPLAProblem.contInterface++;
    ...
    void MoveAttributeMutationSameComponent(ArchitectureSolution solution) {
        ...
        JavaRandomGenerator rng = new JavaRandomGenerator();
    ...
    void MoveMethodMutationSameComponent(ArchitectureSolution architectureSolution) {
    ...
    void MoveOperationMutation(ArchitectureSolution solution) {
    ...
    void AddClassMutation(ArchitectureSolution solution) {
            ...
            if (!searchForGeneralizations(sourceClass) &&
                ...
                if (getRng().nextInt(0, 1) == 0) { //attribute
                    ...
                    if (!attrClassList.isEmpty()) {
                        Class newClass = sourceComp.createClass("Class" + OPLAProblem.contClass, false);
                        OPLAProblem.contClass++;
                        //TODO: allComponents
    ...
    void AddManagerClassMutation(ArchitectureSolution solution) {
        ...
        if (!interfacesComp.isEmpty()) {
            ...
            if (!interfaceMethods.isEmpty()) {
                ...
                Package newComponent = arch.createPackage("Package" + OPLAProblem.contComponent + getPackageSuffix(sourceComp));
                OPLAProblem.contComponent++;

                Interface newInterface = newComponent.createInterface("Interface" + OPLAProblem.contInterface);
                OPLAProblem.contInterface++;
    ...
    void FeatureMutation(ArchitectureSolution solution) {
        ...
        if (!allComponents.isEmpty()) {
            ...
            if (concernsSelectedComp.size() > 1) { // = somente para testes (sic)
                ...
                if (allComponentsForConcern.isEmpty()) {
                    targetPackage = arch.createPackage("Package" + OPLAProblem.contComponent + getPackageSuffix(selectedComponent));
                    OPLAProblem.contComponent++;
                } else {
